...
class GUI():
    ...
    def iniciarSesion(self):
        ...
        while self.sesion:
            try:
                ...
                if datos.lower() == 'iniciar conexion':
                    print('Peticion de conexion recibida por ', direccion)
                    sock.sendto('aceptado'.encode('utf-8'), direccion)
                    print('Solicitud aceptada')
                
                elif datos.lower() == 'sincronizar':
                    sock.sendto(self.hora.getTiempo().encode('utf-8'), direccion)
                    print('Hora enviada a servidor de tiempo')
                    print('Esperando recibir actualizacion de hora...')
    ...
    def __init__(self):
        ...
        self.root = Tk()
        self.root.title('Practica 4 - Servidor 2')
